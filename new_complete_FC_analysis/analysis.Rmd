...
mice_count <- all_day1 %>%
  ...
  select(Genotype, Treatment, Sex, Age) %>%
...
ggplot(data = day0_12_months_HNs, aes(x = Time, y = len, color = Genotype)) +
  ...
  geom_errorbar(aes(ymin = len - se, ymax = len + se), width = 0.2, size = 0.5) +
  theme_bw() + 
  labs(title = 'Day 0 12 Months Old HNs Learning Curves by Genotype, Sex, and Treatment', x = 'Sound Component', 
    y = 'Mean Percent Time Freezing', 
    legend = 'Genotype') +
  scale_x_continuous(breaks = c(1, 2, 3))
...
ggplot(data = day0_12_months_nonHNs, aes(x = Time, y = len, color = Genotype)) +
  ...
  geom_errorbar(aes(ymin = len - se, ymax = len + se), width = 0.2, size = 0.5) +
  theme_bw() + 
  labs(title = 'Day 0 12 Months Old nonHNs Learning Curves by Genotype, Sex, and Treatment', x = 'Sound Component', 
    y = 'Mean Percent Time Freezing', 
    legend = 'Genotype') +
  scale_x_continuous(breaks = c(1, 2, 3))
...
ggplot(data = day1_relevant_data, aes(factor(Genotype), Pct.Total.Time.Freezing)) +
  ...
  labs(title='Day 1 Contextual Test by Genotype and Treatment', y = 'Percent Freezing Time', x='Genotype') + 
  theme(axis.text.x = element_text(angle = 90))
...
ggplot(data = day1_relevant_data, aes(factor(Genotype), Pct.Total.Time.Freezing)) +
  ...
  facet_grid(.~Sex) + 
  geom_jitter(height = 0, width = 0.3, aes(color = Genotype), alpha = 0.25) +
  labs(title='Day 1 Contextual Test by Sex and Treatment', y = 'Percent Freezing Time', x='Genotype') + 
  theme(axis.text.x = element_text(angle = 90))
